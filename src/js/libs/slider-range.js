...
export function sliderRange() {
    ...
    // Конфигурация форматов для разных типов полей
    const fieldConfigs = {
      'price': {
        format: value => Math.round(value).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " "),
        step: 10,
        parse: value => parseInt(value.replace(/\s/g, ''), 10) || 0
      },
      'power': {
        format: value => Math.round(value).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " "),
        step: 1,
    ...
    for (let index = 0; index < slidersRange.length; index++) {
      const fieldType = minInputs[index].dataset.fieldType || 'price';
      const config = fieldConfigs[fieldType];
      
      const initialMaxValue = config.parse(maxInputs[index].value) || 100000;
      const initialMinValue = config.parse(minInputs[index].value) || 0;
      ...
      noUiSlider.create(slidersRange[index], {
        ...
        start: [initialMinValue, initialMaxValue],
        connect: true,
        step: config.step,
        format: {
          to: value => value,
      ...
      function updateInputs(values) {
        minInputs[index].value = config.format(values[0]);
        maxInputs[index].value = config.format(values[1]);
      }
    
      function updateSliderFromInputs() {
        const minValue = config.parse(minInputs[index].value) || 0;
        const maxValue = config.parse(maxInputs[index].value) || initialMaxValue;
        ...
        minInputs[index].value = config.format(minValue);
        maxInputs[index].value = config.format(maxValue);
      }
      ...
      minInputs[index].addEventListener('input', function() {
        const unformatted = config.parse(this.value);
        this.value = config.format(unformatted);
      });
      
      maxInputs[index].addEventListener('input', function() {
        const unformatted = config.parse(this.value);
        this.value = config.format(unformatted);
      });
      ...
      buttonResetForm[index]?.addEventListener('click', () => {
        slidersRange[index].noUiSlider.set([0, initialMaxValue]);
        minInputs[index].value = config.format(0);
        maxInputs[index].value = config.format(initialMaxValue);
      });
    
      updateInputs(slidersRange[index].noUiSlider.get());
    }
    
    //атоматическая ширина для инпутов range
